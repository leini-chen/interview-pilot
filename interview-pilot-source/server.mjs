...
async function refreshRuntimeConfig() {
  let file={};
  try { file=parseEnv(await readFile(new URL('.env',import.meta.url),'utf8')); } catch(e) { if(e.code!=='ENOENT')throw new Error('Could not read the server configuration.'); }
...
async function upstream(path, body, multipart=false) {
  const r=await fetch('https://api.openai.com/v1/'+path,{method:'POST',headers:{Authorization:`Bearer ${key}`,...(!multipart?{'Content-Type':'application/json'}:{})},body:multipart?body:JSON.stringify(body),signal:AbortSignal.timeout(60000)});
...
export const server=http.createServer(async(req,res)=>{
  // Local development server: no public exposure, cross-origin callers, or arbitrary proxying.
  const host=req.headers.host||'';
  if(!['127.0.0.1','localhost','[::1]'].some(h=>host===`${h}:${port}`)){send(res,403,{error:'Local access only.'});return;}
  if(req.headers.origin&&req.headers.origin!==`http://${host}`){send(res,403,{error:'Cross-origin requests are not allowed.'});return;}
  ...
  if(pathname.startsWith('/api/')){
    ...
    if(!key){send(res,503,{error:'Live AI is not configured. Add OPENAI_API_KEY to the server .env file, save it, and check the connection again.'});return;}
...
if(process.argv[1]&&resolve(process.argv[1])===fileURLToPath(import.meta.url))server.listen(port,'127.0.0.1',()=>console.log(`InterviewPilot: http://127.0.0.1:${port} — live AI ${key?'configured':'not configured'}`));

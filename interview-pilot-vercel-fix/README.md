...
For Vercel deployment, follow [DEPLOY-VERCEL.md](DEPLOY-VERCEL.md). The local server binds to loopback only. It checks Host and Origin, limits request bodies, and serves only explicitly allowed public files. `.env` is excluded from Git and cannot be fetched from the server. This is a private-test app: Vercel adapters are included, but authentication, per-user limits, session authorization, metering, and payments are still needed before public launch. The old Sites static manifest does not deploy this Node backend.
